...
public class SprintsConstants {
	...
	final Criteria criteria1 = new Criteria().and(sName).is(criteriaName.get()).and(stringTechnology).is(criteriaTechnology.get())
			.and(stringStartDate).is(date).and(stringEndDate).is(date);
	final Criteria criteriaNoDates = new Criteria().and(sName).is(criteriaName.get()).and(stringTechnology).is(criteriaTechnology.get());
